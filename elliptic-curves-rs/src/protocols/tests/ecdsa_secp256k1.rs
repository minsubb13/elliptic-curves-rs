...
fn test_ecdsa_secp256k1() {
    ...
    let signature = Ecdsa::<Secp256k1Curve>::signing_message(&alice_private_key, z);
    
    assert_eq!(true, Ecdsa::<Secp256k1Curve>::verifying_message(&alice_public_key, z, &signature));
    assert_eq!(false, Ecdsa::<Secp256k1Curve>::verifying_message(&alice_public_key, wrong_z, &signature));
    assert_eq!(false, Ecdsa::<Secp256k1Curve>::verifying_message(&bob_public_key, z, &signature));
}
